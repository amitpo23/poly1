...
class TestMetaBrain(unittest.TestCase):
    ...
    def test_openbb_component_can_drive_meta_score(self):
        mb = self._make_meta_brain(approved=True, score=0.55)
        mb.openbb_reader.query = lambda question: OpenBBMarketSignal(
            direction="bullish",
            probability=0.67,
            ...
            symbol="NVDA",
            asset_class="stock",
            reason="test openbb trend",
            features={"composite_pct": 0.03},
        )

        decision = mb.synthesize(
            market_id="m1",
            question="Will Nvidia close higher today?",
            poly_prob=0.50,
        )

        self.assertTrue(decision.approved)
        self.assertAlmostEqual(decision.features["weighted_components"]["openbb"], 0.67)
        self.assertEqual(decision.features["openbb_symbol"], "NVDA")
        self.assertIn("openbb:NVDA", decision.signal_sources)
